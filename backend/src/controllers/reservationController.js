...
/**
 * Find all ACTIVE pharmacy administrators assigned
...
const createReservation =
  async (req, res) => {
    try {
        ...
        if (
          ...
          return res
            ...
            .json({
              ...
              message:
                message ||
                'Reservation data violates a database constraint',
            })
        }
      ...
      /* --------------------------------------------------------
         Notify pharmacy administrators

         The reservation transaction has already succeeded.
         Notification failures must not undo it.
      -------------------------------------------------------- */
...
/* ============================================================
   ...
   Cancellation through this endpoint restores stock through
  ...
  async (req, res) => {
    try {
      ...
      /* --------------------------------------------------------
         ...
         Stock must be restored atomically with cancellation.
      -------------------------------------------------------- */
...
/* ============================================================
   ...
   The database RPC performs the reservation completion and
   sale creation atomically.
============================================================ */

const completeReservationWithSale =
  async (req, res) => {
    try {
      const pharmacyId =
        req.pharmaUser
      ...
      /*
       * Load the reservation first so we have the
       * customer_id needed for the notification.
       *
       * The RPC still remains responsible for validating
       * whether the reservation can actually be completed.
       */
      const {
        data:
          reservation,
      ...
      if (
        ...
        console.error(
          'Complete reservation lookup error:',
          reservationLookupError
        )
      ...
      if (error) {
        console.error(
          'Complete reservation w/ sale error:',
          error
        )
        ...
        return res
          ...
          .json({
            success: false,
            message:
              'Failed to complete reservation and record sale',
          })
      }
      ...
      return res
        ...
        .json({
          success: true,
          message:
            'Reservation completed and sale recorded',
          data,
        })
    } catch (error) {
      console.error(
        'Complete reservation w/ sale server error:',
        error
      )
...
const cancelReservation =
  async (req, res) => {
    try {
      ...
      /*
       * Load the reservation before cancellation so we know
       * which pharmacy's administrators should be notified.
       *
       * Ownership is still enforced again by the atomic RPC.
       */
      const {
        data:
          reservation,
      ...
      if (
        ...
        console.error(
          'Customer cancellation reservation lookup error:',
          reservationLookupError
        )
