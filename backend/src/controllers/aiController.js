...
const chat = async (req, res) => {
  try {
    ...
    /*
     * Phase 3:
     * Retrieve approved PharmaLink knowledge.
     */
    ...
    /*
     * Phase 4:
     * Resolve controlled live PharmaLink data.
     *
     * SECURITY:
     * customerId comes only from authenticated middleware.
     * It is never accepted from req.body or generated by Gemini.
     */
    const liveDataContext =
      await resolveCustomerDataContext({
        message: trimmedMessage,
        customerId: req.pharmaUser.user_id,
      })
    ...
    if (error.code === 'AI_KNOWLEDGE_FETCH_ERROR') {
    ...
    if (error.code === 'AI_MESSAGES_FETCH_ERROR') {
      return res.status(500).json({
        success: false,
        message:
    ...
    if (
      ...
      if (liveDataErrorCodes.includes(error.code)) {
        return res.status(500).json({
          success: false,
          message:
